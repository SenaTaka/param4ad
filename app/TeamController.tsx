    ...
    <section className="bg-[#0b1828] border border-[#1a3048] rounded-xl p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-1.5">
          ...
          {connected && status?.ip && (
            <span className="text-[10px] font-mono text-cyan-400/70">{status.ip}</span>
          )}
...
export default function TeamController({ team }: { team: string }) {
  ...
  useEffect(() => {
    setRaspiStatus(null)
    fetch(`/api/command?robot=${selectedRobotId}&team=${team}`)
      .then(r => r.json())
      .then(d => setCommand(d.command))
      .catch(() => {})
    fetch(`/api/params?robot=${selectedRobotId}&team=${team}`)
      .then(r => r.json())
      .then(setParams)
      .catch(() => {})
  }, [selectedRobotId, team])
  ...
  useEffect(() => {
    ...
    return () => clearInterval(id)
  }, [isDirty, selectedRobotId, team])

  // ラズパイステータスを2秒ごとに取得
  useEffect(() => {
    const poll = () => {
      fetch(`/api/status?robot=${selectedRobotId}&team=${team}`)
        .then(r => r.json())
  ...
  const handleSave = async () => {
    ...
    try {
      const r = await fetch(`/api/params?robot=${selectedRobotId}&team=${team}`, {
        method: "POST",
  ...
  const handleCommand = async (cmd: Command) => {
    setCommand(cmd)
    await fetch(`/api/command?robot=${selectedRobotId}&team=${team}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const handleAddRobot = async () => {
    ...
    await fetch(`/api/robots?team=${team}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    // 一覧更新
    const list = await fetch(`/api/robots?team=${team}`).then(r => r.json()).catch(() => robots)
    setRobots(list)
  }

  const handleDeleteRobot = async (id: string) => {
    if (!confirm(`「${robots.find(r => r.id === id)?.name ?? id}」を削除しますか？`)) return
    await fetch(`/api/robots?id=${encodeURIComponent(id)}&team=${team}`, { method: "DELETE" }).catch(() => {})
    const list = await fetch(`/api/robots?team=${team}`).then(r => r.json()).catch(() => robots)
    setRobots(list)
      ...
      <div>
        <div className="flex items-center justify-between gap-2">
          <h1 className="text-2xl font-bold text-white tracking-tight shrink-0 flex items-center gap-2">
            ...
            <span className="px-2 py-0.5 rounded-md bg-cyan-600/20 border border-cyan-500/40 text-cyan-300 text-sm font-mono uppercase">
              {team}
            </span>
          </h1>
